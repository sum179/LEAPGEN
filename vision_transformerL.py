...
class VisionTransformerL(nn.Module):
    ...
    def forward_featuresA(self, x, y, task_id=-1, cls_features=None, train=False):
        # if y is None:
        #     print("check 1")
        x = self.patch_embed(x)
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                for i, block in enumerate(self.blocks):
                    ...
                    if i in self.e_prompt_layer_idx:
                        ...
                        res = self.e_prompt(x, y, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=x[:, 0])
                        # res = self.e_prompt(x, y, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=None)
                        # res = self.e_prompt(x, y, task_id=task_id, prompt_mask=prompt_mask, layer_num=e_prompt_counter, cls_features=None)
                        e_prompt = res['batched_prompt']
                        ...
                        # print("check shape x and dec_embed")
                        # print(x.shape)
                        x = torch.cat((desc_embed, x), dim=1)
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                for i, block in enumerate(self.blocks):
                    ...
                    if i in self.e_prompt_layer_idx:
                        ...
                        #     print("Check forward feature A1")
                        ...
                        # x = torch.cat((x,desc_embed,desc_embed,desc_embed,desc_embed,desc_embed), dim=1)
    ...
    #     new_head = nn.Linear(self.embed_dim, int(self.num_classes/self.num_tasks))
    ...
    def forward_headA1(self, res, task_id, device, pre_logits: bool = False):
        ...
        # print("total prompt len: ",self.total_prompt_len)
        # x = x.mean(dim=1)
        
        # x = x[:, self.total_prompt_len]
        
        # x = (x + res['desc_embed'])/2
        # desc_embed = res['desc_embed']
        ...
        # if not self.training:
        #     print("Check forward head A1")

        x = self.fc_norm(x)
        # out = self.head(x)
        # out = self.head.forward2(x,task_id+1)
        out = self.head.forward2(x,max_t)
        # if task_id:
        #     out = self.head.forward2(x,task_id)
        # else:
        #     out = self.head.forward2(x,max_t)
    ...
    def forward_headA2(self, res, device, pre_logits: bool = False):
        x = res['x']
    ...
    def forward_headC(self, res, aux, device, pre_logits: bool = False):
        x = res['x']
    ...
    def forward_head2(self, res, device, pre_logits: bool = False):
        x = res['x']
        if self.class_token and self.head_type == 'token':
            if self.prompt_pool:
                x = x[:, self.total_prompt_len]
    ...
    def forward0(self, x, task_id=-1, cls_features=None, train=False):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_headA(res, device=x.device)
        # res = self.forward_head2(res, device=x.device)
        return res

    def forwardA(self, x, y, task_id=-1, cls_features=None, train=False, kl_idx=None):
        res = self.forward_featuresA(x, y, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_headA(res, device=x.device)
        # res = self.forward_head2(res, device=x.device)
        return res
    ...
    def forwardA2(self, x, aux, task_id=-1, cls_features=None, train=False):
        res = self.forward_featuresA(x, aux, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_headA2(res, device=x.device)
        # res = self.forward_head2(res, device=x.device)
        return res

    def forwardC(self, x, aux, task_id=-1, cls_features=None, train=False):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_headC(res, aux, device=x.device)
        # res = self.forward_head2(res, device=x.device)
        return res
